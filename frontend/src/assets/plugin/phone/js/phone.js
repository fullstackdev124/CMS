...
// let wssServer = getDbItem("wssServer", null); // eg: techfusion.it
// let WebSocketPort = getDbItem("WebSocketPort", null); // eg: 444 | 444˚
// let ServerPath = getDbItem("ServerPath", null); // eg: /ws

let wssServer = "";
let WebSocketPort = 6061;
let ServerPath = "";
let ServerScheme = "wss";
let ServerURL = "";

let SipUsername = getDbItem("SipUsername", ""); // eg: 1000
let SipPassword = getDbItem("SipPassword", ""); // eg: mypassword
let SipDomain = getDbItem("SipDomain", ""); // eg: techfusion.it

let TransportConnectionTimeout = parseInt(getDbItem("TransportConnectionTimeout", 10)); // The timeout in seconds for the initial connection to make on the web socket port
...
let TransportReconnectionTimeout = parseInt(getDbItem("TransportReconnectionTimeout", 5)); // The time in seconds to wait between WebSocket reconnection attempts.

let VoiceMailSubscribe = (getDbItem("VoiceMailSubscribe", "1") == "1"); // Enable Subscribe to voicemail
let userAgentStr = getDbItem("UserAgentStr", "MonkeY Voice WebRTC Phone"); // Set this to whatever you want.
let hostingPrefix = getDbItem("hostingPrefix", "/monkeyvoice/web/phone/"); // Use if hosting off root directiory. eg: "/phone/" or "/static/"
...
let ChatEngine = getDbItem("ChatEngine", "XMPP"); // Select the chat engine XMPP | SIMPLE

// XMPP Settings
let XmppServer = wssServer;
let XmppWebsocketPath = "/http-bind";
let XmppWebsocketPort = WebSocketPort;
// let XmppServer = getDbItem("XmppServer", ""); // FQDN of XMPP server HTTP service";
// let XmppWebsocketPort = getDbItem("XmppWebsocketPort", "443"); // OpenFire Default : 7443
// let XmppWebsocketPath = getDbItem("XmppWebsocketPath", "/wsxmpp"); // OpenFire Default : /ws
...
function UpdateUI() {
	if ($(window).outerWidth() < 920) {
		...
		if (selectedBuddy == null & selectedLine == null) {
			...
			$("#leftContent").css("margin-left", "0px");
			$("#leftContent").show();
		} else {
		...
		if (selectedBuddy == null & selectedLine == null) {
			...
			$("#leftContent").css("margin-left", "-320px");
			$("#rightContent").css("margin-left", "0px");
			$("#leftContent").show();
			$("#rightContent").show();
...
function AddSomeoneWindow(numberStr) {
	...
	// Type
	html += "<ul class=\"list_radio\" style=\"list-style-type:none\">";
...
function InitUi() {
	...
	leftHTML += "<tr><td class=streamSection style=\"height: 94px\">";

	// Profile User
	leftHTML += "<div class=profileContainer>";
	leftHTML += "<div class=contact id=UserProfile style=\"cursor: default; margin-bottom:16px;\">";
	// remove profile picture from header
	// leftHTML += "<div id=UserProfilePic class=buddyIcon></div>";
	leftHTML += "<span class=settingsMenu><button id=SettingsMenu><i class=\"pi pi-cog\"></i></button></span>";
	...
	// Line
	leftHTML += "<div style=\"margin-left:5px; margin-right:5px; margin-bottom: 8px; border-top:1px solid #383838\"></div>";

	// Action Buttons
	leftHTML += "<div class=\"row-btnSoftphone\" style=\"padding-left:5px; padding-right:5px\">";
...
function ShowMyProfileMenu(obj) {
	...
	// items.push({ icon: "fa fa-wrench", text: lang.device_settings, value: 2 });
...
function CreateUserAgent() {
  if (debugging)
	...
	let sipuri = "sip:" + SipUsername  + "@" + SipDomain + ".cms";
	var options = {
		uri: SIP.UserAgent.makeURI(sipuri),
		transportOptions: {
			server: ServerScheme + "://" + wssServer + ":" + WebSocketPort + "" + ServerPath,
			traceSip: false,
			connectionTimeout: TransportConnectionTimeout,
			keepAliveInterval: 10 // Uncomment this and make this any number greater then 0 for keep alive...
		...
		authorizationUsername: SipUsername + "@" + SipDomain,
		authorizationPassword: SipPassword,
		contactParams: { "transport": ServerScheme },
...
function ReceiveCall(session) {
	...
	if(callerID == null || callerID.trim().length <= 0) callerID = did;

  if (debugging)
  	console.log("New Incoming Call:", callerID + " <" + did + ">");

  var evt = new CustomEvent("badge.missed", {detail: 1, cancelable: true, bubbles: true})
...
function RecieveBlf(notification) {
	...
	<presence
		...
		<contact priority="1">sip:User1@techfusion.it</contact>
	    </tuple>
	    <dm:person />
	</presence>
	*/
...
function ClearMissedBadge(buddy) {
  ...
  var evt = new CustomEvent("badge.missed", {detail: 0, cancelable: true, bubbles: true})
  window.dispatchEvent(evt)
}
...
function ShowDial() {
	...
	html += "<td><button class=dialButtons onclick=\"KeyPress('0')\" style=\"font-size: 24px;\">0</button></td>"
	html += "<td><button class=dialButtons onclick=\"KeyPress('#')\">#</button></td></tr>";
	html += "</table>";

	var margin_left = (320-50)/2 - 15;
	if (EnableVideoCalling)
		margin_left = (320-120)/2 -15;
	html += "<div style=\"display: flex; margin-bottom:15px;margin-top:15px;" + "margin-left: "+margin_left+"px;" + "\">";
	html += "<button class=\"dialButtons\" id=dialAudio style=\"background-color: #067d0f !important; font-size: 26px !important;\" title=\"" + lang.audio_call + "\" onclick=\"DialByLine('audio')\"><i class=\"fa fa-phone\" style=\"padding-left: 4px; padding-top: 4px;\"></i></button>";
	if (EnableVideoCalling) {
		html += "<button class=\"dialButtons\" id=dialVideo style=\"background-color: #067d0f !important; font-size: 21px !important; margin-left:20px\" title=\"" + lang.video_call + "\" onclick=\"DialByLine('video')\"><i class=\"fa fa-video-camera\"></i></button>";
	}
	html += "</div>";
	$("#actionArea").html(html);
	$("#actionArea").show();
...
function AddLineHtml(lineObj) {
	...
	html += "<tr><td class=streamSection style=\"height: 60px;\">";
	...
	// Profile UI
	html += "<div class=contact style=\"cursor: unset; float: left; width: 250px;\">";
	...
	// Calling UI --------------------------------------------------------------------------
	html += "<div id=\"line-" + lineObj.LineNumber + "-calling\" style=\"margin-top: 20px;\">";
	...
	html += "</td></tr>";
	html += "<tr><td class=\"streamSection streamSectionBackground\" style=\"\">";  // background-image:url('" + hostingPrefix + "wp_1.png')
...
function UpdateBuddyList() {
	...
	if (callCount > 0) {
		$("#myContacts").append("<hr style=\"height:1px; background-color:#696969;margin:0;\">");
	}
...
function AddBuddyMessageStream(buddyObj) {
	...
	// Close|Return|Back Button
	html += "<button id=\"contact-" + buddyObj.identity + "-btn-back\" onclick=\"CloseBuddy('" + buddyObj.identity + "')\" class=\"roundButtons softphone_back_button\" title=\"" + lang.back + "\"><i class=\"fa fa-chevron-left\"></i></button> ";
...
function ShowMyProfile() {
	...
	// SIP Account
	// html += "<div class=UiTextHeading onclick=\"ToggleHeading(this,'Configure_Extension_Html')\"><i class=\"fa fa-phone UiTextHeadingIcon\" style=\"background-color:#a93a3a\"></i> " + lang.account + "</div>"
	...
	AccountHtml += "<div><input id=Configure_Account_SipDomain class=UiInputText type=password placeholder='" + lang.eg_sip_domain + "' value='" + getDbItem("SipDomain", "") + "'></div>";
	...
	// 2 Audio & Video
	html += "<div class=UiTextHeading onclick=\"ToggleHeading(this,'Audio_Video_Html')\"><i class=\"fa fa-video-camera UiTextHeadingIcon\" style=\"background-color:#208e3c\"></i> " + lang.audio_video + "</div>"
	...
	// 3 Appearance
	html += "<div class=UiTextHeading onclick=\"ToggleHeading(this,'Appearance_Html')\"><i class=\"fa fa-hand-peace-o UiTextHeadingIcon\" style=\"background-color:#416493\"></i> " + lang.appearance + "</div>"
	...
	buttons.push({
		...
		action: function() {
			...
			// TODO -- This needs to be set in Database
...
function getPicture(buddy, typestr, ignoreCache) {
	if (buddy == "profilePicture") {
		...
		if (dbImg == null) {
			return hostingPrefix + "images/user.png";
		} else {
			return dbImg;
...
function DetectDevices() {
	if (navigator.mediaDevices === undefined) {
		// We are in an origin that doesn't allow media devices to be enumerated.
...
var reconnectXmpp = function() {
	...
	var xmpp_websocket_uri = "https://" + XmppServer + ":" + XmppWebsocketPort + "" + XmppWebsocketPath;
	...
	console.log(xmpp_username + " - " + xmpp_password);
...
$(document).ready(function() {
  ...
  if (options.ServerScheme !== undefined) ServerScheme = options.ServerScheme;
  ...
  console.log("Option", JSON.stringify(options), CallWaitingEnabled);
  ...
  $.getJSON(hostingPrefix + "lang/en.json", function(data) {
    ...
    if (loadAlternateLang == true) {
      ...
      if (userLang != "") {
        ...
        $.getJSON(hostingPrefix + "lang/" + userLang + ".json", function(altdata) {
          lang = altdata;
        }).always(function() {
  ...
  // }, 5000)
  // setTimeout(()=> {
  //   var evt = new CustomEvent("badge.missed", {detail: 2, cancelable: true, bubbles: true})
  //   window.dispatchEvent(evt)
  // }, 10000)
  // setTimeout(()=> {
  //   var evt = new CustomEvent("badge.missed", {detail: 0, cancelable: true, bubbles: true})
  //   window.dispatchEvent(evt)
  // }, 15000)
  // var evt = new CustomEvent("toast.success", {detail: "rrr", cancelable: true, bubbles: true})
  // window.dispatchEvent(evt)
});
