DROP PROCEDURE IF EXISTS `call_log_export`;

CREATE DEFINER=`root`@`localhost` PROCEDURE `call_log_export`(IN sdate DATETIME, edate DATETIME, time_offset VARCHAR(10), cus_id INT)
BEGIN
    ...
    SELECT `contact`.`name` AS `contact_name`, `contact`.`city`, `contact`.`state`, `contact`.`country`,
    ...
    WHERE `log`.`created`>=start_at AND `log`.`created`<=end_at AND `log`.`customerId` = IFNULL(cus_id, `log`.customerId);
END
