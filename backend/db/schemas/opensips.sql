...
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE VIEW `domain` AS SELECT
  `id`,
  CONCAT('tenant_', `id`) as `domain`,
  `settings` as `attrs`,
  `createdAt` as `last_modified`
FROM
  `ecms`.`customer`;
...
CREATE VIEW `subscriber` AS SELECT
  `ecu`.`id`, `ecu`.`username` AS `username`, CONCAT("tenant_", `ecc`.`id`) AS `domain`,
  `ecu`.`password` AS `password`, `ecu`.`email` AS `email_address`,
  md5(CONCAT(`ecu`.`username`, ':', CONCAT("tenant_", `ecc`.`id`), ':', `ecu`.`password`)) AS `ha1`,
  md5(CONCAT(`ecu`.`username`, '@', CONCAT("tenant_", `ecc`.`id`), ':', CONCAT("tenant_", `ecc`.`id`), ':', `ecu`.`password`)) AS `ha1b`,
  TRIM(BOTH ' ' FROM CONCAT(`ecu`.`firstName`, ' ', `ecu`.`lastName`)) AS `rpid`
FROM
  `ecms`.`User` AS `ecu`, `ecms`.`customer` AS `ecc`
WHERE
  `ecu`.`customerId` = `ecc`.`id`;
...
--
-- Table structure for table `tmp_rnumber_routing`
--

DROP TABLE IF EXISTS `tmp_rnumber_routing`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `tmp_rnumber_routing` (
  `id` int NOT NULL AUTO_INCREMENT,
  `toolfree_number` varchar(32) NOT NULL,
  `receiving_number` varchar(32) NOT NULL,
...
--
-- Dumping data for table `tmp_rnumber_routing`
--

LOCK TABLES `tmp_rnumber_routing` WRITE;
/*!40000 ALTER TABLE `tmp_rnumber_routing` DISABLE KEYS */;
/*!40000 ALTER TABLE `tmp_rnumber_routing` ENABLE KEYS */;
UNLOCK TABLES;
...
--
-- Dynamic Route Gateway View over ECMS SIP Gateways Table
--

DROP TABLE  IF EXISTS `dr_gateways`;
DROP VIEW IF EXISTS `dr_gateways`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE VIEW `dr_gateways` AS SELECT
	`sg`.`id` AS 'id',
    CAST(CONCAT(`sg`.`id`, "_", LOWER(TRIM(`sg`.`name`))) AS CHAR(64)) AS 'gwid',
    `sg`.`type` AS 'type',
    CAST(CONCAT(`sg`.`address`, IF(`sg`.`port` IS NOT NULL, CONCAT(":", `sg`.`port`), ":5060")) AS CHAR(128)) AS 'address',
    `sg`.`digits_strip` AS 'strip',
    `sg`.`pri_prefix` AS 'pri_prefix',
    CAST(CONCAT("cid=", `sg`.`customerId`) AS CHAR (255) CHARSET LATIN1) AS `attrs`,
    `sg`.`probe_mode` AS 'probe_mode',
    `sg`.`state` AS 'state',
    `sg`.`socket` AS 'socket',
    `sg`.`description` AS 'description'
FROM
	`ecms`.`sip_gateways` AS sg
WHERE
	`sg`.`whitelisted` = 1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2022-01-10 14:13:48
