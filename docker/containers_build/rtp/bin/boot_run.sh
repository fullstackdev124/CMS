...
CONT_IP=`ip addr | grep 'state UP' -A2 | tail -n1 | awk '{print $2}' | cut -f1  -d'/'`
...
# Configure RTPProxy
# Enable those for bridge networking
# sed -i "s/{{IF_IP}}/${CONT_IP}/g" /etc/rtpengine.conf
# sed -i "s/{{BRIDGE_IPS}}/${CONT_IP}!${HOST_IP}/g" /etc/rtpengine.conf
# And this for host networking
sed -i "s/{{IF_IP}}/${HOST_IP}/g" /etc/rtpengine.conf
sed -i "s/{{INSTANCE_DOMAIN}}/${INSTANCE_DOMAIN}/g" /etc/rtpengine.conf
sed -i "s/{{RTPPROXY_CTRL_PORT}}/${RTPPROXY_CTRL_PORT}/g" /etc/rtpengine.conf
sed -i "s/{{ADVERTISED_RANGE_LAST}}/${ADVERTISED_RANGE_LAST}/g" /etc/rtpengine.conf
sed -i "s/{{ADVERTISED_RANGE_FIRST}}/${ADVERTISED_RANGE_FIRST}/g" /etc/rtpengine.conf
