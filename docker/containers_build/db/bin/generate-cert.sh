#!/bin/sh -e
DEFAULT_ORG="domain.dfl"
DEFAULT_PASS="ParantDIOr0x!*"
DEFAULT_CA_CN="Private CA"
DEFAULT_CLIENT_CN="sbc"
...
create_ca () {
	...
	openssl genrsa -des3 -out ${CAKEY} -passout pass:${DEFAULT_PASS} 4096 > /dev/null
	if [ $? -ne 0 ];
	then
		echo "Failed"
		exit 1
	...
	openssl req -new -config ${CACFG} -x509 -days 9999 -key ${CAKEY} -out ${CACERT} -passin pass:${DEFAULT_PASS} > /dev/null
	if [ $? -ne 0 ];
	then
		echo "Failed"
		exit 1
...
create_cert () {
	...
	openssl x509 -req -days 365 -in ${base}.csr -CA ${CACERT} -CAkey ${CAKEY} -set_serial 01 -out ${base}.crt -passin pass:${DEFAULT_PASS} > /dev/null
	if [ $? -ne 0 ];
	then
		echo "Failed"
		exit 1
...
certificates. It  is still recommended to obtain certificates from a recognized 
Certificate Authority and to develop an understanding how SSL certificates work. 
...
OPTIONS:
  ...
  -o  Output filename base (defaults to sbc)
...
To create a CA and a server (pbx.mycompany.com) cert with output in /tmp:
  generate_cert.sh -C pbx.mycompany.com -O "My Company" -d /tmp

This will create a CA cert and key as well as sbc.pem and the the two
files that it is made from: sbc.crt and sbc.key. Copy sbc.pem
and ca.crt somewhere (like /etc/sbc) and set tlscertfile=/etc/sbc.pem
...
OUTPUT_BASE=cert # Our default cert basename
CERT_MODE=server
